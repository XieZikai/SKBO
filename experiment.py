...
def check_model_test_problem(problem, BO, max_iter=50, save_result=True, comparison=False, path=path, acq='ucb',
                             kappa=2.576):
    ...
    if save_result:
        ...
        if BO == BayesianOptimization:
            ...
            anchor_error_record.to_csv(
                os.path.join(path, 'sklearn_anchor_error_record_vanilla_{}.csv'.format(problem.name)))
        else:
            if optimizer.for_comparison:
                result_linear_custom.to_csv(
                    os.path.join(path, 'sklearn_result_test_comparison_{}.csv'.format(problem.name)))
...
for comparison in comparisons:
    ...
    check_model_test_problem(photocatalysis_problem, SurrogateKernelBayesianOptimization, comparison=comparison,
                             acq=acq, kappa=kappa)
